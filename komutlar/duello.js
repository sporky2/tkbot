...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
	...
	if (!opponent) return message.channel.send(new Discord.RichEmbed().setDescription(`${client.emojis.get(client.emojiler.x)} | **Hata:** Oynamak istediğin üyeyi etiketlemelisin!`).setColor(message.guild.me.displayColor));

	if (opponent.bot) return message.channel.send(new Discord.RichEmbed().setDescription(`${client.emojis.get(client.emojiler.x)} | **Hata:** Botlar ile oynayamazsın!`).setColor(message.guild.me.displayColor));
	if (opponent.id === message.author.id) return message.channel.send(new Discord.RichEmbed().setDescription(`${client.emojis.get(client.emojiler.x)} | **Hata:** Kendi kendine oynayamazsın!`).setColor(message.guild.me.displayColor));
	if (this.fighting.has(message.channel.id)) return message.channel.send(new Discord.RichEmbed().setDescription(`${client.emojis.get(client.emojiler.x)} | **Hata:** Her kanalda eşzamanlı tek oyun oynanabilir!`).setColor(message.guild.me.displayColor));
		...
		try {
			if (!opponent.bot) {
				...
				if (!verification) {
					...
					return message.channel.send(new Discord.RichEmbed().setDescription(`${client.emojis.get(client.emojiler.x)} | **Hata:** İstek kabul edilmedi!`).setColor(message.guild.me.displayColor));
				}
			}
			let userHP = 500;
			let oppoHP = 500;
